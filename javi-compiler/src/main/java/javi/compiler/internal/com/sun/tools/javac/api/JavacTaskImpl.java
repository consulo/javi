...
import javi.compiler.internal.com.sun.source.tree.*;
...
import javi.compiler.internal.com.sun.tools.javac.main.*;
...
import javi.compiler.internal.com.sun.tools.javac.processing.AnnotationProcessingError;
...
import javi.compiler.internal.com.sun.tools.javac.util.Log.PrefixKind;
...
public class JavacTaskImpl extends BasicJavacTask {
    ...
    private javi.api.tools.JavaFileManager fileManager;
    private DeferredCompletionFailureHandler dcfh;
    private Locale locale;
    private Map<javi.api.tools.JavaFileObject, JCCompilationUnit> notYetEntered;
    ...
    private void prepareCompiler(boolean forParse) {
        if (used.getAndSet(true)) {
            ...
            if (forParse) {
                ...
                for (javi.api.tools.JavaFileObject file: args.getFileObjects())
    ...
    private Iterable<? extends CompilationUnitTree> parseInternal() {
        try {
            ...
            for (JCCompilationUnit unit: units) {
                javi.api.tools.JavaFileObject file = unit.getSourceFile();
    ...
    @Override @DefinedBy(Api.COMPILER_TREE)
    public Iterable<? extends javi.api.tools.JavaFileObject> generate() {
        Pair<Iterable<? extends javi.api.tools.JavaFileObject>, Throwable> result =  invocationHelper(() -> generate(null));
        if (result.snd == null) {
            return result.fst;
        }
        throw new IllegalStateException(result.snd);
     ...
     */
    public Iterable<? extends javi.api.tools.JavaFileObject> generate(Iterable<? extends Element> classes) {
        final ListBuffer<javi.api.tools.JavaFileObject> results = new ListBuffer<>();
        try {
    ...
    public Type parseType(String expr, TypeElement scope) {
        ...
        compiler = JavaCompiler.instance(context);
        javi.api.tools.JavaFileObject prev = compiler.log.useSource(null);
