...
import javi.api.lang.model.element.NestingKind;
import javi.api.lang.model.element.Modifier;
import javi.api.lang.model.element.Element;

import javi.api.tools.FileObject;
import javi.api.tools.JavaFileManager.Location;

import static javi.api.tools.StandardLocation.SOURCE_OUTPUT;
import static javi.api.tools.StandardLocation.CLASS_OUTPUT;

import javi.api.annotation.processing.Filer;
import javi.compiler.internal.com.sun.tools.javac.code.Lint;
...
import javi.compiler.internal.com.sun.tools.javac.code.Symtab;
...
public class JavacFiler implements Filer, Closeable {
     ...
     */
    private class FilerOutputFileObject extends javi.api.tools.ForwardingFileObject<javi.api.tools.FileObject> {
        private boolean opened = false;
        private ModuleSymbol mod;
        private String name;

        FilerOutputFileObject(ModuleSymbol mod, String name, javi.api.tools.FileObject fileObject) {
            super(fileObject);
            this.mod = mod;
            this.name = name;
        }
    ...
    private class FilerOutputJavaFileObject extends FilerOutputFileObject implements javi.api.tools.JavaFileObject {
        private final javi.api.tools.JavaFileObject javaFileObject;
        FilerOutputJavaFileObject(ModuleSymbol mod, String name, javi.api.tools.JavaFileObject javaFileObject) {
            super(mod, name, javaFileObject);
            this.javaFileObject = javaFileObject;
        }

        @DefinedBy(Api.COMPILER)
        public javi.api.tools.JavaFileObject.Kind getKind() {
            return javaFileObject.getKind();
        }

        @DefinedBy(Api.COMPILER)
        public boolean isNameCompatible(String simpleName,
                                        javi.api.tools.JavaFileObject.Kind kind) {
     ...
     */
    private class FilerInputFileObject extends javi.api.tools.ForwardingFileObject<javi.api.tools.FileObject> {
        FilerInputFileObject(javi.api.tools.FileObject fileObject) {
            super(fileObject);
        }
    ...
    private class FilerInputJavaFileObject extends FilerInputFileObject implements javi.api.tools.JavaFileObject {
        private final javi.api.tools.JavaFileObject javaFileObject;
        FilerInputJavaFileObject(javi.api.tools.JavaFileObject javaFileObject) {
    ...
    private class FilerOutputStream extends FilterOutputStream {
        ModuleSymbol mod;
        String typeName;
        javi.api.tools.FileObject fileObject;
         ...
         * binary file
         */
        FilerOutputStream(ModuleSymbol mod, String typeName, javi.api.tools.FileObject fileObject) throws IOException {
    ...
    private class FilerWriter extends FilterWriter {
         ...
         * text file
         */
        FilerWriter(ModuleSymbol mod, String typeName, javi.api.tools.FileObject fileObject) throws IOException {
    ...
    javi.api.tools.JavaFileManager fileManager;
     ...
     * synchronized.
     */
    private final Set<javi.api.tools.FileObject> initialInputs;

    /**
     * Logical names of all created files.  This set must be
     * synchronized.
     */
    private final Set<javi.api.tools.FileObject> fileObjectHistory;
     ...
     * insertion order.
     */
    private final Map<ModuleSymbol, Map<String, javi.api.tools.JavaFileObject>> generatedClasses;
     ...
     * insertion order.
     */
    private Set<javi.api.tools.JavaFileObject> generatedSourceFileObjects;
    ...
    JavacFiler(Context context) {
        ...
        fileManager = context.get(javi.api.tools.JavaFileManager.class);
    ...
    public javi.api.tools.JavaFileObject createSourceFile(CharSequence nameAndModule,
    ...
    @Override @DefinedBy(Api.ANNOTATION_PROCESSING)
    public javi.api.tools.JavaFileObject createClassFile(CharSequence nameAndModule,
                                                         Element... originatingElements) throws IOException {
        Pair<ModuleSymbol, String> moduleAndClass = checkOrInferModule(nameAndModule);
        return createSourceOrClassFile(moduleAndClass.fst, false, moduleAndClass.snd);
    }

    private Pair<ModuleSymbol, String> checkOrInferModule(CharSequence moduleAndPkg) throws javi.api.annotation.processing.FilerException {
        ...
        if (slash == (-1)) {
            ...
            if (defaultTargetModule == null) {
                throw new javi.api.annotation.processing.FilerException("Cannot determine target module.");
            }
        ...
        if (explicitModule == null) {
            throw new javi.api.annotation.processing.FilerException("Module: " + module + " does not exist.");
        }

        if (!modules.isRootModule(explicitModule)) {
            throw new javi.api.annotation.processing.FilerException("Cannot write to the given module.");
        }

        return Pair.of(explicitModule, pkg);
    }

    private javi.api.tools.JavaFileObject createSourceOrClassFile(ModuleSymbol mod, boolean isSourceFile, String name) throws IOException {
        ...
        javi.api.tools.JavaFileObject.Kind kind = (isSourceFile ?
                                    javi.api.tools.JavaFileObject.Kind.SOURCE :
                                    javi.api.tools.JavaFileObject.Kind.CLASS);

        javi.api.tools.JavaFileObject fileObject =
    ...
    @Override @DefinedBy(Api.ANNOTATION_PROCESSING)
    public javi.api.tools.FileObject createResource(javi.api.tools.JavaFileManager.Location location,
        ...
        if (fileObject instanceof javi.api.tools.JavaFileObject)
            return new FilerOutputJavaFileObject(msym, null, (javi.api.tools.JavaFileObject)fileObject);
        else
            return new FilerOutputFileObject(msym, null, fileObject);
    }

    private void locationCheck(javi.api.tools.JavaFileManager.Location location) {
        if (location instanceof javi.api.tools.StandardLocation) {
            javi.api.tools.StandardLocation stdLoc = (javi.api.tools.StandardLocation) location;
    ...
    @Override @DefinedBy(Api.ANNOTATION_PROCESSING)
    public javi.api.tools.FileObject getResource(javi.api.tools.JavaFileManager.Location location,
                                                 CharSequence moduleAndPkg,
    ...
    private Tuple3<javi.api.tools.JavaFileManager.Location, ModuleSymbol, String> checkOrInferModule(javi.api.tools.JavaFileManager.Location location,
                                                                                                     CharSequence moduleAndPkg,
        ...
        if (slash == (-1)) {
            ...
            if (defaultTargetModule == null) {
                throw new javi.api.annotation.processing.FilerException("No module specified and the location is either " +
        ...
        if (multiModuleLocation) {
            ...
            throw new javi.api.annotation.processing.FilerException("Module specified but the location is neither " +
    ...
    private void checkName(String name) throws javi.api.annotation.processing.FilerException {
        checkName(name, false);
    }

    private void checkName(String name, boolean allowUnnamedPackageInfo) throws javi.api.annotation.processing.FilerException {
        if (!SourceVersion.isName(name) && !isPackageInfo(name, allowUnnamedPackageInfo)) {
            if (lint)
                log.warning(Warnings.ProcIllegalFileName(name));
            throw new javi.api.annotation.processing.FilerException("Illegal name " + name);
        }
    }
    ...
    private void checkNameAndExistence(ModuleSymbol mod, String typename, boolean allowUnnamedPackageInfo) throws javi.api.annotation.processing.FilerException {
        ...
        if (alreadySeen) {
            ...
            throw new javi.api.annotation.processing.FilerException("Attempt to recreate a file for type " + typename);
        }
        if (lint && existing != null) {
            log.warning(Warnings.ProcTypeAlreadyExists(typename));
        }
        if (!mod.isUnnamed() && !typename.contains(".")) {
            throw new javi.api.annotation.processing.FilerException("Attempt to create a type in unnamed package of a named module: " + typename);
        }
    }

    private boolean containedInInitialInputs(String typename) {
        // Name could be a type name or the name of a package-info file
        javi.api.tools.JavaFileObject sourceFile = null;
     ...
     */
    private void checkFileReopening(javi.api.tools.FileObject fileObject, boolean forWriting) throws javi.api.annotation.processing.FilerException {
        if (isInFileObjectHistory(fileObject, forWriting)) {
            if (lint)
                log.warning(Warnings.ProcFileReopening(fileObject.getName()));
            throw new javi.api.annotation.processing.FilerException("Attempt to reopen a file for path " + fileObject.getName());
        }
        if (forWriting)
            fileObjectHistory.add(fileObject);
    }

    private boolean isInFileObjectHistory(javi.api.tools.FileObject fileObject, boolean forWriting) {
        if (forWriting) {
            for(javi.api.tools.FileObject veteran : initialInputs) {
                try {
        ...
        for(javi.api.tools.FileObject veteran : fileObjectHistory) {
            if (fileManager.isSameFile(veteran, fileObject)) {
                return true;
            }
        }
    ...
    public Set<javi.api.tools.JavaFileObject> getGeneratedSourceFileObjects() {
        return generatedSourceFileObjects;
    }

    public Map<ModuleSymbol, Map<String, javi.api.tools.JavaFileObject>> getGeneratedClasses() {
    ...
    public void setInitialState(Collection<? extends javi.api.tools.JavaFileObject> initialInputs,
    ...
    private void closeFileObject(ModuleSymbol mod, String typeName, FileObject fileObject) {
        ...
        if ((typeName != null)) {
            ...
            javi.api.tools.JavaFileObject javaFileObject = (javi.api.tools.JavaFileObject)fileObject;
